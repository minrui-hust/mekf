...
// MeasurementModel
template <typename _Derived, typename _Measurement> struct MeasurementModel {
  ...
  auto measure(const Variable<_State> &state, const Variable<_Noise> &noise,
    ...
    return static_cast<const _Derived &>(*this).measure(
        static_cast<const _State &>(state), static_cast<_Noise>(noise),
        jac_state, jac_noise);
  }
};

// Updater, used to update an RandomVariable by measurement and MeasurmentModel
template <typename _Derived> struct Updater {
  ...
  auto update(
    ...
    return static_cast<const _Derived &>(*this).update(
        state, static_cast<_Measurement &>(measurement), noise,
...
struct CovUpdater : public Updater<CovUpdater> {
  ...
  auto update(
    ...
    measurement.mean() = measurement_model.measure(
        state.mean(), NoiseVariable::Identity(), H, L);
    measurement.uncertainty() = H * state.uncertainty() * H.transpose() +
...
struct SqrtCovUpdater : public Updater<SqrtCovUpdater> {
  ...
  auto update(
      ...
      const MeasurementModel<_Model, _Measurement> &measurement_model) const {
    using StateVariable = _StateVariable;
    // using StateUncertainty = _StateUncertainty<typename
    // StateVariable::Scalar, StateVariable::DoF>;
    using StateRandomVariable =
        RandomVariable<_StateVariable, _StateUncertainty>;
    using StateTangent = typename StateVariable::Tangent;
    ...
    HP_LN.template topRows<StateDof>() =
        state.uncertainty().matrixU() * H.transpose();
    HP_LN.template bottomRows<NoiseDof>() =
        noise.uncertainty().matrixU() * L.transpose();

    // Calculate Sqrt-Covariance based on QR-decomposition
    measurement.uncertainty() =
        HP_LN.householderQr()
            .matrixQR()
            .template topRightCorner<MeasurementDof, MeasurementDof>();
    ...
    Jacobian<StateVariable, MeasurementVariable> dP =
        K * measurement.uncertainty().matrixL();
    ...
    state_new.mean() = state.mean().rplus(dx);
    state_new.uncertainty() =
        (dx_cov.matrixU() * dx.rjac().transpose())
            .householderQr()
            .matrixQR()
