...
template <typename _Derived> struct Interpolator {};
...
struct LinearInterpolator : Interpolator<LinearInterpolator> {
  ...
  auto
  interpolate(const manif::ArrayBase<_State> &state, InterpolateInfo &info,
    ...
    Scalar ratio =
        Scalar(info.dist_curr) / Scalar(info.dist_next + info.dist_curr);
    ...
    if (jac) {
      ...
      interp_state = state.element(info.curr).rplus(
          ratio * state.element(info.next).rminus(state.element(info.curr),
                                                  jac_d_a, jac_d_b),
          jac_o_a, jac_o_d);
      jac->template middleCols<SingleState::DoF>(info.curr * SingleState::DoF) =
          jac_o_a + ratio * jac_o_d * jac_d_a;
      jac->template middleCols<SingleState::DoF>(info.next * SingleState::DoF) =
          ratio * jac_o_d * jac_d_b;
    } else {
      interp_state = state.element(info.curr).rplus(
          ratio * state.element(info.next).rminus(state.element(info.curr)));
    }
