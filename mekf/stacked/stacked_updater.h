...
struct StackedMeasurementModel
    : public MeasurementModel<
          StackedMeasurementModel<_SingleModel, _N, _Interpolator>,
  ...
  auto measure(const Variable<_State> &state, const Variable<_Noise> &noise,
    ...
    auto interp_state = interpolator_.interpolate(
        static_cast<const _State &>(state), info, opt_jac_interp);
    ...
    auto measurement = single_model_.measure(interp_state, noise,
                                             opt_jac_state_single, jac_noise);

    // finally compose jacobian, valid info is store in info
    if (jac_state) {
      // only set value which is valid, leave others unchanged (usually not
      // zero)
      jac_state->template middleCols<ValidSize>(info.valid_idx) =
          jac_state_single *
          jac_interp.template middleCols<ValidSize>(info.valid_idx);
    }
...
struct StackedCovUpdater : public Updater<StackedCovUpdater> {
  ...
  auto update(const RandomVariable<_StateVariable, _StateUncertainty> &state,
              ...
              const MeasurementModel<_Model, _Measurement> &measurement_model,
              InterpolateInfo &info) const {
    using StateVariable = _StateVariable;
    using StateUncertainty =
        _StateUncertainty<typename StateVariable::Scalar, StateVariable::DoF>;
    using StateRandomVariable =
    ...
    // using MeasurementUncertainty = Covariance<typename _Measurement::Scalar,
    // _Measurement::DoF>;
    using MeasurementRandomVariable =
        RandomVariable<MeasurementVariable, Covariance>;

    using NoiseVariable = _NoiseVariable;
    // using NoiseUncertainty = _NoiseUncertainty<typename
    // NoiseVariable::Scalar, NoiseVariable::DoF>; using NoiseRandomVariable =
    // RandomVariable<_NoiseVariable, _NoiseUncertainty>;
    ...
    measurement.mean() = static_cast<const _Model &>(measurement_model)
                             .measure(state.mean(), noise.mean(), info, H, L);

    auto h = H.template middleCols<ValidSize>(info.valid_idx);
    measurement.uncertainty() =
        h *
            state.uncertainty().template block<ValidSize, ValidSize>(
                info.valid_idx, info.valid_idx) *
            h.transpose() +
    ...
    Jacobian<StateVariable, MeasurementVariable> xz_covariance =
        state.uncertainty().template middleCols<ValidSize>(info.valid_idx) *
        h.transpose();

    // Kalman gain
    Jacobian<StateVariable, MeasurementVariable> K =
        xz_covariance * measurement.uncertainty().inverse();

    // Error-state, in tangent space
    StateTangent dx =
        K * measurement_sample.rminus(measurement.mean()).coeffs();

    // Error-state covariance
    StateUncertainty dx_cov =
        state.uncertainty() - K * measurement.uncertainty() * K.transpose();

    // Merge error-state to global state
    StateRandomVariable state_new;
    state_new.mean() = state.mean().rplus(dx);
    state_new.uncertainty() =
        dx.rjac() * 0.5 * (dx_cov + dx_cov.transpose()) * dx.rjac().transpose();
...
struct StackedMeasurementModelList<std::tuple<_Model...>, _N, _Interpolator> {
  using type =
      std::tuple<StackedMeasurementModel<_Model, _N, _Interpolator>...>;
};

template <typename _List, size_t _N, typename _Interpolator>
using StackedMeasurementModelList_t =
    typename StackedMeasurementModelList<_List, _N, _Interpolator>::type;
