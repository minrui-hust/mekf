...
struct StackedTransitionModel
    : public TransitionModel<StackedTransitionModel<_SingleModel, _N>> {
public:
  ...
  auto
  predict(const manif::ArrayBase<_State> &state, const Variable<_Input> &input,
          const Variable<_Noise> &noise,
          OptJacobianRef<typename _State::Element, typename _State::Element>
              jac_state = {},
          OptJacobianRef<typename _State::Element, _Noise> jac_noise = {})
      const { // Note: jacobian is of single state
    using State = typename _State::LieGroup;

    constexpr size_t RemainRepSize = State::RepSize - State::Element::RepSize;

    State next_state;
    next_state.element(0) = single_model_.predict(state.element(0), input,
                                                  noise, jac_state, jac_noise);
    next_state.coeffs().template tail<RemainRepSize>() =
        state.coeffs().template head<RemainRepSize>();
...
struct StackedCovTransformer : public Transformer<StackedCovTransformer> {
  template <typename _StateVariable,
            ...
            typename _Model>
  auto transform(const RandomVariable<_StateVariable, _StateUncertainty> &state,
    ...
    state_new.mean() = static_cast<const _Model &>(model).predict(
        state.mean(), input, noise.mean(), A, L);

    state_new.uncertainty().template topLeftCorner<SingleDoF, SingleDoF>() =
        A * state.uncertainty().template topLeftCorner<SingleDoF, SingleDoF>() *
            A.transpose() +
    ...
    if (RemainDoF != 0) {
      ...
      state_new.uncertainty()
          .template bottomRightCorner<RemainDoF, RemainDoF>() =
          state.uncertainty().template topLeftCorner<RemainDoF, RemainDoF>();

      // fill cov between current state and remained states
      auto cov =
          A *
          state.uncertainty().template topLeftCorner<SingleDoF, RemainDoF>();
      state_new.uncertainty().template topRightCorner<SingleDoF, RemainDoF>() =
          cov;
      state_new.uncertainty()
          .template bottomLeftCorner<RemainDoF, SingleDoF>() = cov.transpose();
    }
