...
// TransitionModel
template <typename _Derived> struct TransitionModel {
  template <typename _State, typename _Input, typename _Noise>
  typename _State::LieGroup
  predict(const Variable<_State> &state, const Variable<_Input> &input,
          const Variable<_Noise> &noise,
          OptJacobianRef<_State, _State> jac_state = {},
          OptJacobianRef<_State, _Noise> jac_noise = {}) const {
    return static_cast<const _Derived &>(*this).predict(
...
template <typename _Derived> struct Transformer {
  ...
  auto transform(const RandomVariable<_StateVariable, _StateUncertainty> &state,
    ...
    return static_cast<const _Derived &>(*this).transform(
        state, static_cast<const _Input &>(input), noise,
...
struct CovTransformer : public Transformer<CovTransformer> {
  ...
  auto transform(const RandomVariable<_StateVariable, _StateUncertainty> &state,
    ...
    state_new.mean() =
        model.predict(state.mean(), input, NoiseVariable::Identity(), A, L);
    state_new.uncertainty() = A * state.uncertainty() * A.transpose() +
...
struct SqrtCovTransformer : public Transformer<SqrtCovTransformer> {
  ...
  auto transform(const RandomVariable<_StateVariable, _StateUncertainty> &state,
    ...
    AP_LN.template topRows<StateDof>() =
        state.uncertainty().matrixU() * A.transpose();
    AP_LN.template bottomRows<NoiseDof>() =
