...
void PartialUpdate(const RandomVariable<_PartialState, Covariance> &prior,
  ...
  Eigen::Matrix<Scalar, PartialDof, RemainDof> Pxy =
      0.5 * (full_state.uncertainty()
                 .template topRightCorner<PartialDof, RemainDof>() +
             full_state.uncertainty()
                 .template bottomLeftCorner<RemainDof, PartialDof>()
                 .transpose());
  ...
  Eigen::Matrix<Scalar, PartialDof, PartialDof> Pxx_new =
      posterior.uncertainty();
  Eigen::Matrix<Scalar, PartialDof, RemainDof> Pxy_new =
      Pxx_new * Pxx_inv * Pxy;
  Eigen::Matrix<Scalar, RemainDof, RemainDof> Pyy_new =
      Pyy + Pxy.transpose() * Pxx_inv *
                (Pxx_new * Pxx_inv.transpose() -
                 Eigen::Matrix<Scalar, PartialDof, PartialDof>::Identity()) *
                Pxy;

  full_state.uncertainty().template topLeftCorner<PartialDof, PartialDof>() =
      Pxx_new;
  full_state.uncertainty().template topRightCorner<PartialDof, RemainDof>() =
      Pxy_new;
  full_state.uncertainty().template bottomLeftCorner<RemainDof, PartialDof>() =
      Pxy_new.transpose();
