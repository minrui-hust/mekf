...
#include "manif/include/manif/manif.h"

namespace ekf {

// variable manif::LieGroup element
template <typename _Derived>
using Variable = manif::LieGroupBase<_Derived>;

template <typename _T, size_t _N>
using VariableArray = manif::Array<_T, _N>;
...
struct RandomVariable {
  ...
  RandomVariable(const Variable& mean, const Uncertainty& uncertainty) : mean_(mean), uncertainty_(uncertainty) {}
...
struct Uncertainty {
  ...
  template <typename _EigenDerived>
  static _Derived FromSqrtCovariance(const Eigen::MatrixBase<_EigenDerived>& m) {
    _Derived uncertainty;
...
struct Covariance : public Uncertainty<Covariance<_Scalar, _dim>>, public SquareMatrix<_Scalar, _dim> {
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
...
using Covarianced = Covariance<double, _dim>;

template <size_t _dim>
using Covariancef = Covariance<float, _dim>;
...
template <size_t _dim>
using SqrtCovarianced = SqrtCovariance<double, _dim>;

template <size_t _dim>
using SqrtCovariancef = SqrtCovariance<float, _dim>;
